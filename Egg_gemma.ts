...
const strip = light.createNeoPixelStrip(pins.D0, stripLength, NeoPixelMode.RGBW)
...
pixel.setColor(pixel.rgb(0, 255, 0));
...
forever(function ()
{
    ...
    pixels.forEach(function (eggPixel: EggPixel) {
...
function pixelProcess(eggPixel: EggPixel)
{
    if (eggPixel.timer > 0) {
...
function pixelCreate(eggPixel: EggPixel) 
{
    if(eggPixel.mode == 0){
...
function stripReset() 
{
    ...
    for (x = 0; x < pixelsMiddle.length; x++){
        pixels[i] = new EggPixel(2, pixelsMiddle.getUint8(x))
        i++
    }
